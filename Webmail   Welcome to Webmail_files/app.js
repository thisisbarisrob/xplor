...
function rcube_webmail()
{
  ...
  this.add_message_row = function(uid, cols, flags, attop)
  {
    ...
    if (cols.subject) {
      ...
      if (rcmail.env.skin == 'elastic') {
        cols.subject = '<span onmouseover="rcube_webmail.long_subject_title(this,'+(message.depth+1)+')" tabindex="-1"><span>'+cols.subject+'</span>';
      } else {
        cols.subject = '<a href="' + this.url(action, query) + '" onclick="return rcube_event.keyboard_only(event)"' + ' onmouseover="rcube_webmail.long_subject_title(this,'+(message.depth+1)+')" tabindex="-1"><span>'+cols.subject+'</span></a>';
      }
    }
  ...
  this.toggle_flagged_status = function(flag, a_uids)
  {
    ...
    if (a_uids.includes(win.frameElement.contentWindow.rcmail.env.uid * 1)) {
      // only update flag on currently displayed message if it's part of the uids being updated
  ...
  this.ksearch_query_results = function(results, search, reqid)
  {
    ...
    if (reqid && this.ksearch_pane.data('reqid') == reqid) {
      ...
      if (is_framed) {
        try {
          parent.$('iframe :not([sandbox])').each(function() {
...
{
  $('iframe :not([sandbox])').each(function() {
    var frame = $(this);
