        ...
        var open_func = function(e) {
            ...
            select.popover('dispose')
                ...
                .on('shown.bs.popover', function() {
                    // clear sanity check for popover not displaying 
                    if (rcmail.popover_flip_sanity) delete rcmail.popover_sanity;
                ...
                .on('show.bs.popover', function() {
                  // on small screens, if the popover is set to overlap the parent element the
                  // popover will not display. we can trigger it by forcibly calling show.
                  rcmail.popover_flip_sanity = this;
                  setTimeout(function() {
                    if (rcmail.popover_flip_sanity) {
                      // If the client is slow and this gets called when the popover has already
                      // been shown, the show call does nothing and no harm is done
                      $(rcmail.popover_flip_sanity).popover('show');
                      delete rcmail.popover_flip_sanity;
                    }
                  },100);
                })
                .popover('show');
